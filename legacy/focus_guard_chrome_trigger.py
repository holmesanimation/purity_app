# LEGACY STANDALONE ENTRYPOINT – retained for backwards compatibility.
# New deployments should use app.py + reminder_dialog.py + chrome_dialog.py.
...
def save_belief_scripture_config(config):
    ...
    clean_config = normalize_belief_config(config)
    with CONFIG_PATH.open("w", encoding="utf-8") as f:
        json.dump(config_to_json_ready(clean_config), f, indent=2, ensure_ascii=False)
    return clean_config
...
def is_browser_running():
    return _is_process_running("chrome.exe") or _is_process_running("msedge.exe")


def chrome_watch_loop():
    """Legacy entry point – wraps ProcessOpenWatcher for backward compatibility."""
    ProcessOpenWatcher(
        is_running=is_browser_running,
        poll_seconds=CHROME_CHECK_SECONDS,
        cooldown_seconds=CHROME_TRIGGER_COOLDOWN_SECONDS,
        on_opened=lambda: trigger_focus_popup(is_long_break=False),
    ).run_forever()
...
# -----------------------------
def timer_loop():
    minutes_since_long = 0

    while True:
        time.sleep(REGULAR_INTERVAL_MINUTES * 60)
        minutes_since_long += REGULAR_INTERVAL_MINUTES

        is_long = minutes_since_long >= LONG_BREAK_EVERY_MINUTES
        if is_long:
            minutes_since_long = 0

        trigger_focus_popup(is_long)
...
def create_tray_image():
    ...
    draw.ellipse((10, 10, 54, 54), fill="lightblue", outline="black")
    draw.rectangle((22, 24, 42, 46), fill="white", outline="black")
...
def show_reset_now(icon, item):
    threading.Thread(
        target=lambda: trigger_focus_popup(is_long_break=False),
        daemon=True
    ).start()
...
def run_tray():
    icon = pystray.Icon(
        ...
        "Focus Guard",
        menu=pystray.Menu(
            pystray.MenuItem("Reset now", show_reset_now),
            pystray.MenuItem(
                "Configure beliefs + scriptures",
                lambda icon, item: threading.Thread(
                    target=lambda: open_belief_config_window(None),
                    daemon=True
                ).start()
            ),
            pystray.MenuItem("Reload", reload_app),
...
def start_focus_guard():
    # Start chrome watcher in background (if enabled), then run timer loop
    if CHROME_TRIGGER_ENABLED:
        threading.Thread(target=chrome_watch_loop, daemon=True).start()

    #trigger_focus_popup(is_long_break=False)
    timer_loop()


threading.Thread(target=start_focus_guard, daemon=True).start()
